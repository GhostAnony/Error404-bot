...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
        ...
        const red = chalk.hex('#FF003C');
        const redDark = chalk.hex('#C70039');
        const orange = chalk.hex('#FF6B00');
        const white = chalk.hex('#F5F5F5');
        const gray = chalk.hex('#A0A0A0');
        const green = chalk.hex('#00FF88');

        const top = red.bold('╭─────────────『 ERROR404 SYSTEM 』─────────────╮');
        const bottom = red.bold('╰─────────────────────────────────────────────╯');
        const divider = redDark('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');

        const title = red.bold('           ⚠ ERROR404 AUTHENTICATION ⚠');

        const qr = orange('⌬') + ' ' + white.bold('MODALITÀ [1] • Accesso tramite QR');
        const code = orange('⌬') + ' ' + white.bold('MODALITÀ [2] • Accesso tramite Codice');

        const info = [
            gray('➜ Inizializzazione protocollo di accesso...'),
            gray('➜ Verifica connessione ai server...'),
            gray('➜ Seleziona una modalità di autenticazione.'),
            '',
            redDark.italic('ERROR404 CORE • Secure Login System')
        ];
...
${prompt}`);
        ...
        if (!/^[1-2]$/.test(opzione)) {
            console.log(`
${red.bold('⚠ ERRORE 404 • INPUT NON VALIDO')}

${white('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')}

${gray('➜ Valori consentiti:')} ${green.bold('1')} ${white('oppure')} ${green.bold('2')}
${gray('➜ Non utilizzare lettere')}
${gray('➜ Non utilizzare simboli')}
${gray('➜ Non utilizzare spazi')}

${orange.bold('Accesso negato dal sistema ERROR404.')}
...
${chalk.hex('#FF003C').bold('╭────『 ERROR404 LINK SYSTEM 』────╮')}

${chalk.hex('#F5F5F5')('➜ Inserisci il numero WhatsApp')}
${chalk.hex('#FF6B00')('➜ Esempio: +393471234567')}

${chalk.hex('#00FF88').bold('⌬ error404 ➜ ')}
`);
        ...
        console.log(
            `\n${chalk.hex('#FF6B00').bold('⌛ Generazione codice di collegamento in corso...')}`
        );

        setTimeout(async () => {
            let codeBot = await conn.requestPairingCode(addNumber, 'ERROR404');
            codeBot = codeBot?.match(/.{1,4}/g)?.join('-') || codeBot;

            console.log(`
${chalk.hex('#FF003C').bold('╭────『 ERROR404 PAIRING CODE 』────╮')}

${chalk.hex('#F5F5F5').bold('🔑 CODICE DI ACCESSO')}

${chalk.hex('#00FF88').bold(`      ${codeBot}`)}

${chalk.hex('#A0A0A0')('Inserisci questo codice sul dispositivo da collegare.')}
                ...
                try {
                    readdirSync(dirName).forEach(file => {
                        ...
                        try {
                            ...
                            if (
                                stats.isFile() &&
                                (Date.now() - stats.mtimeMs) > 2 * 60 * 1000
                            ) {
                                unlinkSync(filePath);
                            }
                        } catch {}
...
async function connectionUpdate(update) {
    ...
    if (!global.isLogoPrinted) {
        const schemaColoriErrore = [
            '#FF3131', '#FF4500', '#FF8C00', '#FF9F00', '#FF4500', '#FF3131'
        ];
        ...
        errore404.forEach((line, i) => {
            const color = schemaColoriErrore[i] || schemaColoriErrore[schemaColoriErrore.length - 1];
...
(async () => {
    try {
        ...
        console.log(chalk.hex('#2ECC71').bold(`ERROR-BOT connesso correttamente`));
    } catch (error) {
