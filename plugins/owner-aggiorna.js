...
let handler = async (m, { conn, command, usedPrefix }) => {
  if (/^debugplugin$/i.test(command)) {
    ...
    if (!debugId || !global.updateDebugErrors[debugId]) {
      return conn.reply(m.chat, '*⚠️ [𝖲𝖸𝖲𝖳𝖤𝖬] 𝖲𝖾𝗌𝗌𝗂𝗈𝗇𝖾 𝖽𝖾𝖻𝗎𝗀 𝗇𝗈𝗇 𝗍𝗋𝗈𝗏𝖺𝗍𝖺 𝗈 𝗌𝖼𝖺𝖽𝗎𝗍𝖺.*', m)
    }

    const item = global.updateDebugErrors[debugId]

    const fullMsg =
`⚠️ *𝖤𝖱𝖱𝖮𝖱-𝟦𝟢𝟦 𝖲𝖸𝖲𝖳𝖤𝖬 𝖣𝖤𝖡𝖴𝖦*

📁 *𝖥𝗂𝗅𝖾:* ${item.file}
⚡ *𝖤𝖼𝖼𝖾𝗓𝗂𝗈𝗇𝖾:* ${item.message}
...
> 𝚵𝐑𝐑𝐎𝐑 𝚩𝚯𝐓 ♻️`
  ...
  try {
    await conn.reply(m.chat, '🔄 *[𝖲𝖸𝖲𝖳𝖤𝖬]* 𝖠𝗇𝖺𝗅𝗂𝗌𝗂 𝖽𝖾𝗂 𝗉𝖺𝖼𝗀𝗁𝖾𝗍𝗍𝗂 𝗋𝖾𝗆𝗈𝗍𝗂 𝗂𝗇 𝖼𝗈𝗋𝗌𝗈...', m)
    ...
    const updatedFiles = diffStatus
      ...
      .map(line => {
        ...
        if (status === 'A') {
          ...
          return `📥 [𝖭𝖤𝖶] ${oldPath} (+${stats.plus}/-${stats.minus})`
        }

        if (status === 'D') {
          const stats = statMap[oldPath] || { plus: 0, minus: 0 }
          return `🗑️ [𝖣𝖤𝖫] ${oldPath} (+${stats.plus}/-${stats.minus})`
        }

        const stats = statMap[oldPath] || { plus: 0, minus: 0 }
        return `🛠️ [𝖬𝖮𝖣] ${oldPath} (+${stats.plus}/-${stats.minus})`
      })
    ...
    let resultMsg = `🟢 *𝖮𝖵𝖤𝖱𝖶𝖱𝖨𝖳𝖤 𝖢𝖮𝖬𝖯𝖫𝖤𝖳𝖮 𝖢𝖮𝖭 𝖲𝖴𝖢𝖢𝖤𝖲𝖲𝖮*`

    if (updatedFiles.length > 0) {
      resultMsg += `\n\n📦 *𝖬𝖮𝖣𝖴𝖫𝖨 𝖨𝖭𝖨𝖤𝖳𝖳𝖳𝖠𝖳𝖨:* ${updatedFiles.length}\n\n${updatedFiles.join('\n')}`
    } else {
      resultMsg += '\n\nℹ️ *𝖭𝖾𝗌𝗌𝗎𝗇𝖺 𝗇𝗎𝗈𝗏𝖺 𝗋𝗂𝗅𝖾𝗏𝖺𝗓𝗂𝗈𝗇𝖾 𝖽𝗂 𝖼𝗈𝖽𝗂𝖼𝖾.*'
    }
          ...
          buttons: [
            {
              ...
              buttonText: { displayText: '🔧 𝖠𝗇𝖺𝗅𝗂𝗌𝗂 𝖫𝗈𝗀 𝖢𝗈𝗆𝗉𝗅𝖾𝗍𝗈' },
              type: 1
            }
          ],
          headerType: 1
